...
/* USER CODE BEGIN DbgOutputTraces */
  ...
  {
    /* This is intentionally a lossy debug sink. The callback releases the
     * middleware source buffer after it is copied or deliberately dropped;
     * it does not mean every byte has reached the physical SWO pin. */
    (void)NodeSwo_Write(p_data, size);
    if (cb != NULL)
    {
      cb();
    }
