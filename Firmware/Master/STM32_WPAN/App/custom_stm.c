...
/* USER CODE BEGIN PD */
#undef CHAR_VALUE_LEN_CONSTANT
#define CHAR_VALUE_LEN_CONSTANT CHAR_VALUE_LEN_VARIABLE
#define CUSTOM_STM_MAX_PAYLOAD_SIZE 244U
      ...
      switch (blecore_evt->ecode)
      {
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE :
          /* USER CODE BEGIN EVT_BLUE_GATT_READ_PERMIT_REQ_BEGIN */
          {
            ...
            read_perm_req = (aci_gatt_read_permit_req_event_rp0*)blecore_evt->data;
            if ((read_perm_req->Attribute_Handle == (CustomContext.CustomPipestattxHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)) ||
                (read_perm_req->Attribute_Handle == (CustomContext.CustomPipecfgrwHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)))
            {
              return_value = SVCCTL_EvtAckFlowEnable;
              (void)aci_gatt_permit_read(read_perm_req->Connection_Handle, 0x00U, 0x00U, 0x0000U);
              break;
            }
          }
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  SizePipedatatx = CUSTOM_STM_MAX_PAYLOAD_SIZE;
  SizePipectrlrx = 180U;
  SizePipectrltx = 180U;
  SizePipestattx = 180U;
  SizePipecfgrw = 180U;
...
tBleStatus Custom_STM_App_Update_Char_Variable_Length(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload, uint8_t size)
{
  ...
  if ((pPayload == 0) || (size == 0U) || (size > CUSTOM_STM_MAX_PAYLOAD_SIZE)) {
    return BLE_STATUS_INVALID_PARAMS;
  }
