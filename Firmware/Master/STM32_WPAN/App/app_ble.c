...
{
  8, AD_TYPE_COMPLETE_LOCAL_NAME, 'H', 'U', 'B', '0', '0', '0', '1',  /* Complete name */
...
void APP_BLE_Init(void)
{
  ...
  EXO_LOG("[BLE][APP][INIT] shci cfg_links=%u max_att_mtu=%u mblocks=%u\r\n",
          (unsigned)CFG_BLE_NUM_LINK,
          (unsigned)CFG_BLE_MAX_ATT_MTU,
          (unsigned)CFG_BLE_MBLOCK_COUNT);
  ...
  g_phone_connection_handle = 0xFFFFU;
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      if (p_disconnection_complete_event->Connection_Handle == g_phone_connection_handle)
      {
        g_phone_connected = 0U;
        g_phone_connection_handle = 0xFFFFU;
        EXO_LOG("[BLE][APP][DISC] phone disconnect handle=0x%04X reason=0x%02X\r\n",
                (unsigned)p_disconnection_complete_event->Connection_Handle,
                (unsigned)p_disconnection_complete_event->Reason);
      }
      else
      {
        EXO_LOG("[BLE][APP][DISC] leaf disconnect handle=0x%04X reason=0x%02X\r\n",
                (unsigned)p_disconnection_complete_event->Connection_Handle,
                (unsigned)p_disconnection_complete_event->Reason);
        exo_hub_central_client_on_disconnection_complete(p_disconnection_complete_event->Connection_Handle,
                                                         p_disconnection_complete_event->Reason);
        if (g_phone_connected != 0U)
        {
          BleApplicationContext.Device_Connection_Status = APP_BLE_CONNECTED_SERVER;
          BleApplicationContext.BleApplicationContext_legacy.connectionHandle = g_phone_connection_handle;
        }
      }
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      if (p_meta_evt->subevent == HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE)
      {
        ...
        const uint8_t raw_adv_len = (p_event_pckt->plen > 1U) ? (uint8_t)(p_event_pckt->plen - 1U) : 0U;
        if (raw_adv_len < 11U)
        {
          EXO_LOG("[BLE][APP][DISC] adv report short len=%u\r\n",
                  (unsigned)raw_adv_len);
        }
        else if (raw_adv[0] != 0U)
        {
          Advertising_Report_t adv_report;
          uint8_t data_len;
          const uint8_t num_reports = raw_adv[0];
          adv_report.Event_Type = raw_adv[1];
          adv_report.Address_Type = raw_adv[2];
          memcpy(adv_report.Address, &raw_adv[3], sizeof(adv_report.Address));
          data_len = raw_adv[9];
          if (data_len > 31U)
          {
            EXO_LOG("[BLE][APP][DISC] adv report invalid len=%u count=%u\r\n",
                    (unsigned)data_len,
                    (unsigned)num_reports);
          }
          else if ((uint8_t)(10U + data_len) >= raw_adv_len)
          {
            EXO_LOG("[BLE][APP][DISC] adv report truncated raw_len=%u adv_len=%u count=%u\r\n",
                    (unsigned)raw_adv_len,
                    (unsigned)data_len,
                    (unsigned)num_reports);
          }
          ...
          {
            adv_report.Length_Data = data_len;
            adv_report.Data = &raw_adv[10];
            adv_report.RSSI = raw_adv[10U + data_len];
//            EXO_LOG("[BLE][APP][DISC] adv report count=%u rssi=%d len=%u type=0x%02X\r\n",
//                    (unsigned)num_reports,
//                    (int)(int8_t)adv_report.RSSI,
//                    (unsigned)adv_report.Length_Data,
//                    (unsigned)adv_report.Event_Type);
            hci_le_advertising_report_event(1U, &adv_report);
          }
        }
      }
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          if (g_leaf_client_connecting != 0U && (p_connection_complete_event->Status != 0U || p_connection_complete_event->Role == 0U))
          {
            g_leaf_client_connecting = 0U;
            EXO_LOG("[BLE][APP][DISC] leaf connect handle=0x%04X status=0x%02X role=%u addr=%02X:%02X:%02X:%02X:%02X:%02X\r\n",
                    (unsigned)p_connection_complete_event->Connection_Handle,
                    (unsigned)p_connection_complete_event->Status,
                    (unsigned)p_connection_complete_event->Role,
                    (unsigned)p_connection_complete_event->Peer_Address[5],
                    (unsigned)p_connection_complete_event->Peer_Address[4],
                    (unsigned)p_connection_complete_event->Peer_Address[3],
                    (unsigned)p_connection_complete_event->Peer_Address[2],
                    (unsigned)p_connection_complete_event->Peer_Address[1],
                    (unsigned)p_connection_complete_event->Peer_Address[0]);
            exo_hub_central_client_on_connection_complete(1U,
          ...
          else
          {
            ...
            EXO_LOG("[BLE][APP][DISC] phone connect handle=0x%04X status=0x%02X role=%u leaf_pending=%u addr=%02X:%02X:%02X:%02X:%02X:%02X\r\n",
                    (unsigned)p_connection_complete_event->Connection_Handle,
                    (unsigned)p_connection_complete_event->Status,
                    (unsigned)p_connection_complete_event->Role,
                    (unsigned)g_leaf_client_connecting,
                    (unsigned)p_connection_complete_event->Peer_Address[5],
                    (unsigned)p_connection_complete_event->Peer_Address[4],
                    (unsigned)p_connection_complete_event->Peer_Address[3],
                    (unsigned)p_connection_complete_event->Peer_Address[2],
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        {
          const aci_gap_proc_complete_event_rp0 *p_evt =
              (const aci_gap_proc_complete_event_rp0 *)p_blecore_evt->data;
          aci_gap_proc_complete_event(p_evt->Procedure_Code,
                                      p_evt->Status,
                                      p_evt->Data_Length,
                                      p_evt->Data);
          break;
        ...
        {
          const aci_att_read_by_group_type_resp_event_rp0 *p_evt =
              (const aci_att_read_by_group_type_resp_event_rp0 *)p_blecore_evt->data;
          aci_att_read_by_group_type_resp_event(p_evt->Connection_Handle,
                                                p_evt->Attribute_Data_Length,
                                                p_evt->Data_Length,
                                                p_evt->Attribute_Data_List);
          break;
        }

        case ACI_ATT_READ_BY_TYPE_RESP_VSEVT_CODE:
        {
          const aci_att_read_by_type_resp_event_rp0 *p_evt =
              (const aci_att_read_by_type_resp_event_rp0 *)p_blecore_evt->data;
          aci_att_read_by_type_resp_event(p_evt->Connection_Handle,
                                          p_evt->Handle_Value_Pair_Length,
                                          p_evt->Data_Length,
                                          p_evt->Handle_Value_Pair_Data);
          break;
        }

        case ACI_GATT_NOTIFICATION_VSEVT_CODE:
        {
          const aci_gatt_notification_event_rp0 *p_evt =
              (const aci_gatt_notification_event_rp0 *)p_blecore_evt->data;
          aci_gatt_notification_event(p_evt->Connection_Handle,
                                      p_evt->Attribute_Handle,
                                      p_evt->Attribute_Value_Length,
                                      p_evt->Attribute_Value);
          break;
        }

        case ACI_GATT_INDICATION_VSEVT_CODE:
        {
          const aci_gatt_indication_event_rp0 *p_evt =
              (const aci_gatt_indication_event_rp0 *)p_blecore_evt->data;
          aci_gatt_indication_event(p_evt->Connection_Handle,
                                    p_evt->Attribute_Handle,
                                    p_evt->Attribute_Value_Length,
                                    p_evt->Attribute_Value);
          break;
        ...
        {
          const aci_gatt_proc_complete_event_rp0 *p_evt =
              (const aci_gatt_proc_complete_event_rp0 *)p_blecore_evt->data;
          aci_gatt_proc_complete_event(p_evt->Connection_Handle,
                                       p_evt->Error_Code);
          break;
        }
...
void APP_BLE_LeafClientConnecting(void)
{
  g_leaf_client_connecting = 1U;
  BleApplicationContext.Device_Connection_Status = APP_BLE_LP_CONNECTING;
  EXO_LOG("[BLE][APP][DISC] connect state=CONNECTING\r\n");
}
...
void APP_BLE_LeafClientConnectIdle(void)
{
  ...
  EXO_LOG("[BLE][APP][DISC] connect state=IDLE\r\n");
}
...
uint8_t APP_BLE_LeafClientPrepareScan(void)
{
  if (g_phone_connected != 0U)
  {
    BleApplicationContext.Device_Connection_Status = APP_BLE_SCAN;
    EXO_LOG("[BLE][APP][DISC] scan state=SCAN phone-connected handle=0x%04X\r\n",
            (unsigned)g_phone_connection_handle);
    return 1U;
  }
      ...
      BleApplicationContext.Device_Connection_Status == APP_BLE_LP_ADV)
  {
    ...
    {
      EXO_LOG("[BLE][APP][DISC] scan prepare stop-adv failed\r\n");
      return 0U;
    }
    g_leaf_scan_holds_advertising = 1U;
    EXO_LOG("[BLE][APP][DISC] scan prepare stop-adv ok\r\n");
  }
  BleApplicationContext.Device_Connection_Status = APP_BLE_SCAN;
  EXO_LOG("[BLE][APP][DISC] scan state=SCAN\r\n");
  return 1U;
}
...
void APP_BLE_LeafClientScanIdle(void)
{
  if (g_phone_connected != 0U)
  {
    BleApplicationContext.Device_Connection_Status = APP_BLE_CONNECTED_SERVER;
    BleApplicationContext.BleApplicationContext_legacy.connectionHandle = g_phone_connection_handle;
    EXO_LOG("[BLE][APP][DISC] scan idle phone-connected handle=0x%04X\r\n",
            (unsigned)g_phone_connection_handle);
    return;
  }
  if (g_leaf_scan_holds_advertising != 0U &&
      BleApplicationContext.Device_Connection_Status != APP_BLE_LP_CONNECTING)
  {
    g_leaf_scan_holds_advertising = 0U;
    EXO_LOG("[BLE][APP][DISC] scan idle resume adv\r\n");
    Adv_Request(APP_BLE_FAST_ADV);
  }
}
...
/* USER CODE BEGIN Role_Mngt*/
  role |= GAP_CENTRAL_ROLE;
  EXO_LOG("[BLE][APP][DISC] gap role=0x%02X peripheral=%u central=%u cfg_peripheral=%u cfg_central=%u\r\n",
          (unsigned)role,
          (unsigned)((role & GAP_PERIPHERAL_ROLE) != 0U),
          (unsigned)((role & GAP_CENTRAL_ROLE) != 0U),
          (unsigned)BLE_CFG_PERIPHERAL,
          (unsigned)BLE_CFG_CENTRAL);
