...
static mb_errot_t validateAnswer(modbusHandler_t *modH);
static void buildException(uint8_t u8exception, modbusHandler_t *modH);
static mb_errot_t validateRequest(modbusHandler_t *modH);
...
static int16_t getRxBuffer(modbusHandler_t *modH);
static mb_errot_t SendQuery(modbusHandler_t *modH, modbus_t telegram);
...
void ModbusInit(modbusHandler_t *modH)
	...
	if (numberHandlers < MAX_M_HANDLERS)
	{
		...
		modH->xTimerTimeout = xTimerCreate("xTimerTimeout", pdMS_TO_TICKS(modH->u16timeOut), pdFALSE, 0, (TimerCallbackFunction_t) vTimerCallbackTimeout);
...
//			modH->QueueTelegramHandle = osMessageQueueNew(MAX_TELEGRAMS, sizeof(modbus_t), &QueueTelegram_attributes);
		...
		modH->xTimerT35 = xTimerCreate("TimerT35", pdMS_TO_TICKS(T35), pdFALSE, 0, (TimerCallbackFunction_t) vTimerCallbackT35);
...
//		modH->ModBusSphrHandle = osSemaphoreNew(1, 1, &ModBusSphr_attributes);
		...
		#else
			xTaskCreate(StartTaskModbusSlave, "TaskModbusSlave", 128, modH, osPriorityHigh5, &modH->myTaskModbusAHandle);
			//			modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusSlave, modH, &myTaskModbusA_attributes);
#endif
		...
		#else
			xTaskCreate(StartTaskModbusMaster, "TaskModbusMaster", 128, modH, osPriorityHigh5, &modH->myTaskModbusAHandle);
			//			modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusMaster, modH, &myTaskModbusB_attributes);
#endif
		}
		else
...
//extern void modbus_task_callback();
  ...
  if( modH->xTypeHW == TCP_HW )
  {
	...
	for (;;)
	  ...
	  if(modH-> xTypeHW == TCP_HW)
	  {
		...
		mb_errot_t u8exception = validateRequest(modH);
		if (u8exception > 0)
				{
		...
		modH->i8lastError = ERR_NO_ERR;
		if (xSemaphoreTake(modH->ModBusSphrHandle, pdMS_TO_TICKS(500)) == pdFALSE) {
			delay_ms(1);
			continue;
		}
 ...
 * @ingroup loop
 */
mb_errot_t SendQuery(modbusHandler_t *modH, modbus_t telegram)
		{

	uint8_t u8regsno, u8bytesno;
	mb_errot_t error = ERR_NO_ERR;
	xSemaphoreTake(modH->ModBusSphrHandle, pdMS_TO_TICKS(1000)); //before processing the message get the semaphore
	...
	for (;;)
     ...
     else // send a query for USART and USB_CDC
     {
      ...
      if(modH->xTypeHW == USART_HW) //TCP and USB_CDC use different methods to get the buffer
      {
		...
		mb_errot_t u8exception = validateAnswer(modH);
		if (u8exception != 0)
				{
			modH->i8state = COM_IDLE;
			modH->i8lastError = u8exception;
 ...
 * @ingroup buffer
 */
mb_errot_t validateAnswer(modbusHandler_t *modH)
		{
...
// check fct code
	...
	return ERR_NO_ERR; // OK, no exception code thrown
}
 ...
 */
mb_errot_t validateRequest(modbusHandler_t *modH)
		{
// check message crc vs calculated crc
...
// transfer buffer to serial line IT
	 ...
	 */
#if defined(STM32H7)  || defined(STM32F3) || defined(STM32L4) || defined(STM32L082xx) || defined(STM32F7) || defined(STM32WB) || defined(STM32G070xx) || defined(STM32F0) || defined(STM32G431xx) || defined(STM32H5) || defined(STM32U5)
...
int8_t process_FC6(modbusHandler_t *modH)
	...
	uint8_t u8CopyBufferSize;
	uint16_t u64val = word(modH->u8Buffer[NB_HI], modH->u8Buffer[NB_LO]);

	modH->u16regs[u16add] = u64val;
...
// write registers
	...
	if (u16regsno < 5) {
		for (i = 0; i < u16regsno; i++)
			...
			u64valsave |= (temp << (16 * i));
		}
		u8CopyBufferSize = modH->u8BufferSize + 2;
		sendTxBuffer(modH);
	}

//	= modH->u16regs[0] | ((uint64_t)modH->u16regs[1] << 16) | ((uint64_t)modH->u16regs[2] << 32) | ((uint64_t)modH->u16regs[3] << 48);
//	write_settings(u64valsave, MODBUS_ADDRESS);
//	modbus_task_callback();
	return u8CopyBufferSize;
}
...
void MODBUS_INIT() {
	...
	ModbusH2.EN_Port = RS485_2_DE_GPIO_Port;
	ModbusH2.EN_Pin = RS485_2_DE_Pin;
	ModbusH2.u16regs = ModbusDATA2;
	ModbusH2.u16regsize = sizeof(ModbusDATA2) / sizeof(ModbusDATA2[0]);
	ModbusH2.xTypeHW = USART_HW;
	xSemaphoreGive(&ModbusH2.ModBusSphrHandle); //Release the semaphore
