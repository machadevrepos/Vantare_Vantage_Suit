...
- **#24 (run-index allocation)** — fixed: `MasterBinarySessionIndex` now warms a boot-time cache (`init_cache()` after FATFS init in `main.c`): reads a persisted 8-byte marker `/SESSIONS/RUNIDX.BIN` (magic "INDX" + last index); if missing/corrupt, enumerates `/SESSIONS` once (single `f_opendir`/`f_readdir` pass parsing `R####`/`TRN####` names) and rewrites the marker. `allocate()` is now a handful of `f_stat`s from `cached_last_+1`, persists the marker at hand-out, and falls back lazily if the card was absent at boot. Crash semantics: an index is never re-issued once handed out (stranded indexes possible after a crash between allocation and archive, matching the designed fix). The `test_binary_first_invariants.py` allocator guard was refined: `FA_CREATE_ALWAYS` allowed only for the RUNIDX.BIN marker, never payload files.
  - **HW regression found 2026-08-22, fixed in `1a14805`**: the first boot on a card without a marker ran the original rebuild as an O(9999×8) `f_stat` storm — minutes of SPI I/O before BLE/superloop start, so the Master appeared dead whenever a card was inserted. The rebuild is now the single-directory-pass described above, and `init_cache()` runs after `PWR_EN` + 200 ms settle + the SPI1 prescaler bump.
